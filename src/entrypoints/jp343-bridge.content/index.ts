  ...
  main() {
    ...
    function ensureProjectExists(entry: PendingEntry): void {
      try {
        ...
        // Channel-Name ist der bevorzugte Projekt-Name
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            // Fuer eingeloggte User: Server-Sync ist PFLICHT
            // Fuer Gaeste: localStorage reicht
            if (userState.isLoggedIn) {
              // Original Video-Titel und URL explizit mitgeben (nicht den Projekt-Namen)
              const serverSuccess = await syncEntryToServer(jp343Entry, userState, originalVideoTitle, originalResourceUrl);
              if (!serverSuccess) {
                // Server-Sync fehlgeschlagen - Entry NICHT als synced markieren!
            ...
            // Entry als synced markieren (nur wenn Server-Sync erfolgreich oder Gast)
