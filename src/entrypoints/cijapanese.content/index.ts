...
// No ads, no iframes, standard HTML5 video events
...
export default defineContentScript({
  ...
  main() {
    ...
    function findVideoElement(): HTMLVideoElement | null {
      // Only Vidstack player videos (real content), not decorative videos (.video-clips, autoplay loop)
    ...
    intervalIds.push(setInterval(() => {
      if (window.location.href !== lastUrl) {
        ...
        // Wait for SPA navigation to complete before looking for new video
        setTimeout(() => {
          if (!isWatchPage()) return;
          const video = findVideoElement();
          if (video) {
