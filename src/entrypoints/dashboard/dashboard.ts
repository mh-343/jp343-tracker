...
// Privileged extension page: reads browser.storage.local directly
...
// Local dailyMinutes for merge with server data (set in refresh())
...
async function doLogin(email: string, password: string): Promise<{ success: boolean; error?: string; userState?: JP343UserState }> {
  try {
    ...
    if (result.success && result.data?.nonce) {
      ...
      // Auto-sync all pending entries immediately after login
      try {
...
async function doLogout(): Promise<void> {
  ...
  refresh();
  // Reset flag after 2s to prevent onChanged from re-triggering login
...
// Normalize field names across different server response formats
...
function setupAuthUI(): void {
  ...
  // Sign Up button is an <a> — intercept for local drawer toggle
...
function renderStats(stats: ExtensionStats): void {
  ...
  if (activeDays > 0) {
    ...
    // Weekly avg: total time / number of calendar weeks with activity
    ...
    // Monthly avg: total time / number of calendar months with activity
...
function renderHeatmap(dailyMinutes: Record<string, number>): void {
  ...
  // Day labels: spacer for month row + Mon/Wed/Fri + spacer for active-days row
...
function renderWeekBars(dailyMinutes: Record<string, number>): void {
  ...
  // Bar area height: container 100px - ~18px value - ~18px label = ~64px
  const BAR_MAX_PX = 64;
...
// Server stats endpoint returns seconds; daily_avg is penalty-weighted (more accurate than local calc)
const CACHED_SERVER_STATS_KEY = 'jp343_cached_server_stats';

// Server as baseline, local wins when higher (unsynced today entries)
...
// Compute stats not covered by applyServerStats (from merged dailyMinutes)
function applyDerivedStats(dailyMinutes: Record<string, number>): void {
  ...
  for (const [date, min] of Object.entries(dailyMinutes)) {
    ...
    // Simplified ISO week number (Thursday-based)
...
function applyServerStats(serverData: ServerStatsResponse): void {
  ...
  // Daily avg from server response (penalty-weighted, more accurate than local calc)
  if (serverData.daily_avg_seconds !== undefined) {
    setText('statDailyAvg', formatStatDuration(serverData.daily_avg_seconds / 60));
  }
  // Heatmap/charts + derived stats from full DB history
  ...
  // Cache server stats for instant display on next reload
  browser.storage.local.set({ [CACHED_SERVER_STATS_KEY]: serverData });
}

// Apply cached server stats immediately to prevent 0h 0min flicker
...
async function refresh(): Promise<void> {
  ...
  try {
  const data = await loadData();
  // Save local dailyMinutes for later merge with server data
  ...
  if (isLoggedIn) {
    // Show cached server stats immediately to prevent flicker (0h -> 1200h)
    await applyCachedServerStats();
    // Render sessions only after server fetch to avoid double-render
    showSessionsLoading();
