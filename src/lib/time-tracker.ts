...
export class TimeTracker {
  ...
  startSession(videoState: VideoState, tabId?: number): TrackingSession {
    ...
    if (this.session && this.session.url === videoState.url) {
      ...
      // Update channel info using same logic as updateSessionChannelInfo
      ...
      // Thumbnail may arrive late (e.g. Crunchyroll iframe race condition)
  ...
  // Cap delta at 5s to prevent jumps from tab switches etc.
  private tick(): void {
  ...
  finalizeSession(): PendingEntry | null {
    ...
    // Float minutes for second-level precision (e.g. 1.33 = 80 seconds)
  ...
  // Only updates if title was not manually edited
  ...
  // Handles late-arriving or corrected channel info (SPA navigation race conditions)
  updateSessionChannelInfo(channelId: string | null, channelName: string | null, channelUrl: string | null): boolean {
    ...
    // YouTube: channelId arrives late due to SPA navigation timing
    const gotNewChannelId = channelId && hadNoChannelId;
    // Netflix/Crunchyroll: channelId is always null, but channelName changes
